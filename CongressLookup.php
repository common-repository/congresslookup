...
function congress_get_default_map_center(){
	return '1600 Pennsylvania Ave, Washington, DC 20500';
}
		...
		if ($congress === false || !get_option('congress_cache') ) {
		    ...
		    if( !empty($congress->officials) ){
                ...
                foreach ($congress->officials as $c) {
                    ...
                    //if( in_array("title", $a) ) {
                        $output .= "<h3 class='legislator'>" . esc_html( $c->name ) . "</h3>";
                    //}
	...
	if($congress_key):
		...
		if( $atts['congress_form'] == 'yes' ){
			$html .= '  <fieldset id="user-details">	
							...
							<input type="text" name="congress_address" id="congress_address' . $id . '" placeholder="' . esc_attr($atts['congress_map_center_placeholder']) . '" value="' . esc_attr($atts['congress_map_center']) . '" />
